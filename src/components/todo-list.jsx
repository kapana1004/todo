...
const Todo = (props) => {
  ...
  return(
   ...
   <div className=' items-center w-[350px]'>
    ...
    <input type='text'  value={inputValue} onKeyDown={changeHandling}  onChange={ (event) => setInputValue(event.target.value)} className=' w-[270px] ml-[20px] resize-none  h-[48px]   placeholder:text-xs text-[13px] rounded-[5px]' placeholder='Create a new todo…' >
      </input>
    </div>
    ...
    <ul className=' w-[327px] ml-[24px] bg-white mt-[20px]  rounded-[5px]'>
       ...
       <CheckBox setFilteredList = {setFilteredList}  filteredList = {filteredList} todoItem={todoItem}  todoList = {todoList} setTodoList={setTodoList} key={index} />
        )
       })}
     
       <div className=' rounded-[5px] w-[327px] h-[50px] flex justify-between p-[20px] items-center' > 
          <span className=' cursor-pointer text-xs text-gray-400'> 5 items left </span>  
          <span className=' cursor-pointer text-xs text-gray-400'>Clear Completed</span>
       </div>
    ...
    <div className=' rounded-[5px] w-[327px] h-[48px]  flex justify-around items-center mt-[20px] ml-[25px] bg-white '> 
          <span className= {`text-[14px] font-sans cursor-pointer ${category==='all'? 'text-blue-600': 'text-gray-700'}`}  onClick={handleFilter}>All</span> 
          <span className= {`text-[14px] font-sans cursor-pointer ${category==='active'? 'text-blue-600': 'text-gray-700'}`} onClick={handlingActive}>Active</span>  
          <span className= {`text-[14px] font-sans cursor-pointer ${category==='completed'? 'text-blue-600': 'text-gray-700'}`} onClick={handleComplete} >Completed</span>
     </div>
...
// footers layout styles doesn`t work out of ul (margin top??)
// once added todo is replacing by another pushed todo